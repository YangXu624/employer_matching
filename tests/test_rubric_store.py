...
def test_loads_provided_rubric_in_fixed_order():
    ...
    assert rubric.level_descriptions[0].competency_id == "career_self_development"
    assert rubric.level_descriptions[0].level == 1


def test_rejects_missing_level_key(tmp_path):
    source = json.loads(Path("employer_match/data/rubric.json").read_text())
    broken = copy.deepcopy(source)
    del broken["career_self_development"]["levels"]["5"]
